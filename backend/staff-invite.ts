...
// Appelée depuis le CRM interne (onglet "Équipe") par un membre de
// l'équipe déjà connecté, pour créer le compte d'un nouveau collègue.
// Vérifie d'abord que l'appelant est bien membre de l'équipe (is_staff())
// avant de créer quoi que ce soit — sinon n'importe quel compte client
// authentifié pourrait s'auto-promouvoir en membre de l'équipe.
// Crée le compte Supabase Auth (mot de passe temporaire), l'ajoute à
// staff_users, puis envoie ce mot de passe par e-mail au nouveau membre.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: isStaff, error: staffErr } = await sbCaller.rpc("is_staff");
    if (staffErr || !isStaff) {
      return new Response(JSON.stringify({ error: "Réservé à l'équipe SF Club Paris." }), { status: 403, headers: CORS });
    }
    ...
    await email(
      ...
      `<p>Bonjour ${nom || ""},</p>
       ...
       <p>Connectez-vous puis pensez à changer ce mot de passe depuis Supabase si besoin.</p>
